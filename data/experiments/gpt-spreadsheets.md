...
This setup is text-only at the moment, so it doesn’t support image generation, but for text-based tasks can help save time. You can provide a list of prompts (previous image) and compose the prompts dynamically according to data using `CONCATENATE` (next image). If you need to force the spreadsheet to recalculate a cell, simply delete its contents and undo the action.
...
- **System Content**: provides the model with guidance on how it should behave throughout the generation. It influences the tone, behavior, and boundaries for the model’s responses.
